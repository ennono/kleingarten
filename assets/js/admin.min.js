...
jQuery(document).ready(function ($) {
    ...
    $('#addRow').on('click', function () {
      ...
      `;
        $('#zaehlerTabelle tbody').append(row);
    });

    $(document).on('click', '.removeRow', function () {
        const rows = $('#zaehlerTabelle tbody tr');
        if (rows.length > 1) {
    ...
    $('#zaehlerForm').on('submit', function (e) {
        e.preventDefault();
        alert('Formular wurde abgeschickt!');
        // Hier könnten die Daten per Ajax gesendet oder verarbeitet werden.
    });
